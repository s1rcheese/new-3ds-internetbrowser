...
PluginView::paint(GraphicsContext* ctx, const IntRect& rect)
{
    ...
    int type = ImageWKC::EColorARGB8888;
    ...
    if (type==ImageWKC::EColorARGB8888) {
        ImageContainerWKC wimg(type, bitmap, rowbytes, size);
        wimg.setHasAlpha(false);
        if (m_isTransparent) {
            if (fmt&WKC_IMAGETYPE_FLAG_HASALPHA)
                wimg.setHasAlpha(true);
        }
        RefPtr<BitmapImage> img = BitmapImage::create(static_cast<NativeImagePtr>(&wimg));
        ctx->drawImage(img.get(), WebCore::ColorSpaceDeviceRGB, framerect.location());
    }
