...
const double pollTimeSeconds = 0.1;  // TODO: adjust

void WebSocketServer::platformInitialize()
{
    m_tcpServerHandler = adoptPtr(new WKCTcpServerHandler(this));
}
...
{
    m_webSocketServer->didAcceptConnection(SocketStreamHandleServer::create(socket, 0));
}
...
void WKCTcpServerHandler::pollTimerCallback(Timer<WKCTcpServerHandler>* timer)
{
    fd_set rfds;
    FD_ZERO(&rfds);
