...
#if PLATFORM(WKC)
// (Just comment)
// We don't wrap these global variables with WKC_DEFINE_STATIC_DOUBLE(),
// instead, initialize them in WKC::WKCWebKitInitialize().
#endif
// Normal delay
...
#if PLATFORM(WKC) && (defined(__ghs__) || defined(__ARMCC_VERSION))
    // Do nothing to avoid unwanted update of :hover/:active.
#else
...
void FrameView::performPostLayoutTasks()
{
    ...
    if (RenderView* root = rootRenderer(this)) {
        if (!root->printing()) {
            ...
            bool resized = !m_firstLayout && parentFrameView() && (currentSize != m_lastViewportSize || currentZoomFactor != m_lastZoomFactor);
#else
...
bool FrameView::doLayoutWithFrameFlattening(bool allowSubtree)
{
    ...
    if (!parentView->m_frame || (parentView != parentView->m_frame->view())) {
        return false;
    }
#endif
...
void FrameView::paintContents(GraphicsContext* p, const IntRect& rect)
{
    ...
    Page* page = m_frame->page();
#ifdef PLATFORM(WKC)
    // NOTE: Just a workaround. Will be unnecessary in the future
    //       since Pagenation is not owned by Page after r126343.
    if (page)
#endif
