...
#if PLATFORM(WKC)
unsigned Page::sessionStorageMemoryConsumptionBytes()
{
    if (!m_sessionStorage) {
        return 0;
    }

    return m_sessionStorage->memoryConsumptionBytes();
}

void Page::clearSessionStorageForAllOrigins()
{
    if (!m_sessionStorage) {
        return;
    }

    m_sessionStorage->clearAllOriginsForDeletion();
}
#endif
