...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_PTR(OverrideSizeMap*, gOverrideHeightMap, 0);
WKC_DEFINE_GLOBAL_PTR(OverrideSizeMap*, gOverrideWidthMap, 0);

WKC_DEFINE_GLOBAL_CLASS_OBJ(bool, RenderBox, s_hadOverflowClip, false);
#else
...
void RenderBox::styleDidChange(StyleDifference diff, const RenderStyle* oldStyle)
{
    ...
    if (hasOverflowClip() && oldStyle && newStyle && layer() && oldStyle->effectiveZoom() != newStyle->effectiveZoom()) {
...
#if PLATFORM(WKC)
LayoutUnit RenderBox::constrainContentBoxLogicalHeightByMinMax(LayoutUnit logicalHeight)
{
    RenderStyle* styleToUse = style();
    if (!styleToUse->logicalMaxHeight().isUndefined()) {
        LayoutUnit maxH = computeContentLogicalHeightUsing(styleToUse->logicalMaxHeight());
        if (maxH != -1)
            logicalHeight = min(logicalHeight, maxH);
    }
    return max(logicalHeight, computeContentLogicalHeightUsing(styleToUse->logicalMinHeight()));
}
#endif
...
bool RenderBox::canBeScrolledAndHasScrollableArea() const
{
    ...
    const RenderBlock* block = toRenderBlock(this);
    int sw = scrollWidth();
...
LayoutUnit RenderBox::computePercentageLogicalHeight(const Length& height)
{
    ...
    // height.
...
#if PLATFORM(WKC)
        LayoutUnit contentBoxHeight = cbstyle->logicalHeight().value();
        if (cb->style()->boxSizing() == BORDER_BOX)
            contentBoxHeight -= cb->borderAndPaddingLogicalHeight();
        contentBoxHeight = max<LayoutUnit>(0, contentBoxHeight);
        result = max<LayoutUnit>(0, cb->constrainContentBoxLogicalHeightByMinMax(contentBoxHeight - cb->scrollbarLogicalHeight()));
#else
        ...
        if (result != -1) {
            ...
            // yet.
            LayoutUnit contentBoxHeight = cb->constrainContentBoxLogicalHeightByMinMax(result - cb->scrollbarLogicalHeight());
            result = max<LayoutUnit>(0, contentBoxHeight);
        }
#else
        if (result != -1)
    ...
    LayoutUnit offsetFromLogicalTopOfFirstPage, bool checkForPerpendicularWritingMode) const
{
    ...
    if (containingBlock->isBox() && style()) {
        if (style()->position() == FixedPosition) {
            if (FrameView* view = document()->view()) {
                if (RenderView* renderView = document()->renderView())
                    return adjustForAbsoluteZoom(view->visibleWidth(), renderView); // but not view->layoutWidth() to enforce non-fixed layout
            }
        }
    }
#endif
...
LayoutUnit RenderBox::containingBlockLogicalHeightForPositioned(const RenderBoxModelObject* containingBlock, bool checkForPerpendicularWritingMode) const
{
    ...
    if (containingBlock->isBox() && style()) {
        if (style()->position() == FixedPosition) {
            if (FrameView* view = document()->view()) {
                if (RenderView* renderView = document()->renderView())
                    return adjustForAbsoluteZoom(view->visibleHeight(), renderView); // but not view->layoutHeight() to enforce non-fixed layout
            }
        }
    }
#endif
