 ...
 * Copyright (c) 2012, 2013, 2014 ACCESS CO., LTD. All rights reserved.
 *
...
LayoutRect RenderTextControlSingleLine::controlClipRect(const LayoutPoint& additionalOffset) const
{
    ...
    if (containerElement()->renderBox())
        clipRect = unionRect(clipRect, containerElement()->renderBox()->frameRect()); // clientBoxRect() to allow `bleeding out' texts.
#else
...
#if PLATFORM(WKC)
bool RenderTextControlSingleLine::itemIsEnabledRespectingGroup(unsigned listIndex) const
{
     if (!listIndex || itemIsSeparator(listIndex))
        return false;
    return true;
