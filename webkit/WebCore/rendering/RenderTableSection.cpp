...
#if ENABLE(WKC_ANDROID_LAYOUT)
    CellSpan dirtiedRows = CellSpan(0, m_grid.size());
    CellSpan dirtiedColumns = CellSpan(0, table()->columns().size());
    if (table()->isSingleColumn()) {
        ...
        if (m_rowPos.size() != (m_grid.size() + 1))
            return;
#endif
