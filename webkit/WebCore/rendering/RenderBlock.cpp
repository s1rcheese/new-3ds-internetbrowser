...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_PTR(ColumnInfoMap*, gColumnInfoMap, 0);
#else
static ColumnInfoMap* gColumnInfoMap = 0;
#endif

#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_PTR(TrackedDescendantsMap*, gPositionedDescendantsMap,  0);
WKC_DEFINE_GLOBAL_PTR(TrackedDescendantsMap*, gPercentHeightDescendantsMap, 0);
#else
...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_PTR(TrackedContainerMap*, gPositionedContainerMap, 0);
WKC_DEFINE_GLOBAL_PTR(TrackedContainerMap*, gPercentHeightContainerMap, 0);
#else
...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_INT(gDelayUpdateScrollInfo, 0);
WKC_DEFINE_GLOBAL_PTR(DelayedUpdateScrollInfoSet*, gDelayedUpdateScrollInfoSet, 0);
#else
...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_BOOL(gColumnFlowSplitEnabled, true);
#else
...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_CLASS_OBJ(bool, RenderBlock, s_canPropagateFloatIntoSibling, false);
#else
...
#if ENABLE(WKC_ANDROID_LAYOUT)
    LayoutUnit oldVisibleWidth = m_visibleWidth;
#endif
...
#if ENABLE(WKC_ANDROID_LAYOUT)
    bool result = oldWidth != logicalWidth() || oldColumnWidth != desiredColumnWidth();
...
{
#if PLATFORM(WKC)
    // Ugh!: workaround for b115899!
    // 140502 ACCESS Co.,Ltd.
    if (!(region && inRenderFlowThread())) {
        return 0;
    }
#else
