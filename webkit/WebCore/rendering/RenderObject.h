...
#if USE(CG) || USE(CAIRO) || USE(SKIA) || PLATFORM(QT) || USE(WKC_CAIRO)
...
#if PLATFORM(WKC)
    void focusRingRects(Vector<IntRect>&);
#endif
...
#if PLATFORM(WKC)
    WKC_DEFINE_GLOBAL_CLASS_OBJ_ENTRY(RenderObjectAncestorLineboxDirtySet*, s_ancestorLineboxDirtySet);
#else
...
#if PLATFORM(WKC)
    WKC_DEFINE_GLOBAL_CLASS_OBJ_ENTRY(bool, s_affectsParentBlock);
#else
