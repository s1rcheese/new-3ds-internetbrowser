...
void RenderLayer::updateLayerPositionsAfterScroll(RenderGeometryMap* geometryMap, UpdateLayerPositionsAfterScrollFlags flags)
{
    ...
    if (flags & HasSeenViewportConstrainedAncestor
        ...
        if (m_repaintRect != renderer()->clippedOverflowRectForRepaint(renderer()->containerForRepaint()))
            return;
        if (m_outlineBox != renderer()->outlineBoundsForRepaint(renderer()->containerForRepaint(), geometryMap))
            return;
#else
...
#  ifdef VN_TEMPORAL_WORKAROUND
        if ( ! fixedPositionContainerLayer ) { return; }
#  else
...
#if PLATFORM(WKC)
    WKC_DEFINE_STATIC_PTR(IntPoint*, previousMousePosition_, new IntPoint());
    IntPoint& previousMousePosition = *previousMousePosition_;
#else
...
void RenderLayer::updateHoverActiveState(const HitTestRequest& request, HitTestResult& result)
{
    ...
    // Disabled the following on WKC, because our mouse-release events may follow touch-release events.
    // The target of such a mouse-release event may be moved or vanished by relayout the clearing causes.
#if !PLATFORM(WKC)
