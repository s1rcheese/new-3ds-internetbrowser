...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_PTR(ContinuationMap*, continuationMap, 0);
#else
...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_PTR(FirstLetterRemainingTextMap*, firstLetterRemainingTextMap, 0);
#else
...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_PTR(ImageQualityController*, gImageQualityController, 0);
#else
    ...
    if (oldStyle && newStyle) {
        ...
        if (hasLayer()
            && (oldStyle->hasAutoZIndex() != newStyle->hasAutoZIndex()
                || oldStyle->zIndex() != newStyle->zIndex()
#if PLATFORM(WKC)
                   // We have modified the definition of the stacking context. See RenderLayer::isStackingContext().
                || ((oldStyle->position() == FixedPosition) != (newStyle->position() == FixedPosition))
#endif
...
#if USE(WKC_CAIRO) && CAIRO_VERSION == CAIRO_VERSION_ENCODE(1, 12, 0)
#  ifdef WKC_CUSTOMER_PATCH_0307364
#  else
            /* To avoid cairo 1.12.0 bug,
               if both inset shadow and border radius are used and
               background style is specified, skip painting shadow. */
