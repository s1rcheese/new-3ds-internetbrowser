...
struct FrameFlatteningLayoutDisallower {
    FrameFlatteningLayoutDisallower(FrameView* frameView)
        ...
        , m_disallowLayout((frameView && frameView->frame() && frameView->frame()->settings()) ? frameView->frame()->settings()->frameFlatteningEnabled() : false)
    {
        if (m_frameView && m_disallowLayout)
