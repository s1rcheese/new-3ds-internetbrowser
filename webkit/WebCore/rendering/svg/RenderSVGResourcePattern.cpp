...
#else
WKC_DEFINE_GLOBAL_CLASS_OBJ(RenderSVGResourceType, RenderSVGResourcePattern, s_resourceType, PatternResourceType);
#endif
    ...
    , m_shouldCollectPatternAttributes(true)
{
    ...
    if (!inited) {
        inited = true;
        s_resourceType = PatternResourceType;
    }
#endif
}
...
{
    return (SVGPatternElement *)RenderSVGResourceContainer::node();
}
...
void RenderSVGResourcePattern::collectPatternAttributes(PatternAttributes& attributes) const
{
    ...
    while (current) {
        ...
        SVGResources* resources = SVGResourcesCache::cachedResourcesForRenderObject(current);
        current = resources ? (RenderSVGResourcePattern *)resources->linkedResource() : 0;
    }
}
