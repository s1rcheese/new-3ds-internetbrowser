...
#if PLATFORM(WKC)
    if (parent->isSVGRoot()) {
        TransformationMatrix matrix(object->localToParentTransform().toTransformationMatrix());
        matrix.multiply(TransformationMatrix(toRenderSVGRoot(parent)->localToBorderBoxTransform().toTransformationMatrix()));
        geometryMap.push(object, matrix);
    } else
        geometryMap.push(object, TransformationMatrix(object->localToParentTransform().toTransformationMatrix()));
#else
    if (parent->isSVGRoot()) {
        TransformationMatrix matrix(object->localToParentTransform());
        matrix.multiply(TransformationMatrix(toRenderSVGRoot(parent)->localToBorderBoxTransform()));
        geometryMap.push(object, matrix);
    } else
        geometryMap.push(object, TransformationMatrix(object->localToParentTransform()));
#endif
