...
{
#if PLATFORM(WKC)
    WKC_DEFINE_STATIC_PTR(RenderSVGResourceSolidColor*, s_sharedSolidPaintingResource, 0);
#else
...
#else
    WKC_DEFINE_STATIC_PTR(HashSet<SVGElement*>*, invalidatingDependencies, 0);
    if (!invalidatingDependencies)
        invalidatingDependencies = new HashSet<SVGElement*>();
#endif
...
#if !PLATFORM(WKC)
            if (UNLIKELY(!invalidatingDependencies.get().add(*it).isNewEntry)) {
#else
...
#if !PLATFORM(WKC)
            invalidatingDependencies.get().remove(*it);
#else
