 ...
 * Copyright (c) 2012, 2013, 2015 ACCESS CO., LTD. All rights reserved.
 *
...
{
    bool threedimage = false;

    CachedImage* img = cachedImage();
    if (img && img->hasImage()) {
        Image* image = img->image();
        if (image && image!=Image::nullImage()) {
            if (image->isThreeDImage())
                threedimage = true;
        }
    }

    return RenderBox::requiresLayer() | threedimage;
}
#endif
...
void RenderImage::imageDimensionsChanged(bool imageSizeChanged, const IntRect* rect)
{
    ...
    if (intrinsicSizeChanged) {
        ...
        setWidth(oldwidth);
        setHeight(oldheight);
#if PLATFORM(WKC) && USE(ACCELERATED_COMPOSITING)
            // Tell any potential compositing layers that the image needs updating.
            styleDidChange(StyleDifferenceRecompositeLayer,0);
#endif
    }
