...
struct BidiIsolatedRun {
    BidiIsolatedRun(RenderObject& object, unsigned position, RenderObject& root, BidiRun& runToReplace)
        : object(object)
        , root(root)
...
static inline void notifyObserverWillExitObject(Observer* observer, RenderObject* object)
{
    ...
    if (isIsolated(unicodeBidi) && observer->inIsolate()) {
...
static inline void addPlaceholderRunForIsolatedInline(InlineBidiResolver& resolver, RenderObject* obj, unsigned pos, RenderObject& root)
{
    ASSERT(obj);
...
public:
    ...
    void addFakeRunIfNecessary(RenderObject* obj, unsigned pos, RenderObject& root, InlineBidiResolver& resolver)
    {
    ...
    if (!m_emptyRun && !m_eor.atEnd() && !m_reachedEndOfLine) {
        ...
        while (obj && obj != m_eor.m_obj && obj != endOfLine.m_obj) {
            if (isolateTracker.inIsolate())
                isolateTracker.addFakeRunIfNecessary(obj, start, *m_sor.root(), *this);
            else
