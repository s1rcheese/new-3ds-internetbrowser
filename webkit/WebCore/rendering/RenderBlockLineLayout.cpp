...
static inline void constructBidiRuns(InlineBidiResolver& topResolver, BidiRunList<BidiRun>& bidiRuns, const InlineIterator& endOfLine, VisualDirectionOverride override, bool previousLineBrokeCleanly)
{
    ...
    while (!topResolver.isolatedRuns().isEmpty()) {
        ...
        InlineIterator iter = InlineIterator(isolatedInline, startObj, isolatedRun.position);
...
void RenderBlock::layoutInlineChildren(bool relayoutChildren, LayoutUnit& repaintLogicalTop, LayoutUnit& repaintLogicalBottom)
{
    ...
    if (firstChild()) {
        ...
        bool doTextWrap = settings && viewport.width != ViewportArguments::ValueDeviceWidth &&
            settings->layoutAlgorithm() == Settings::kLayoutFitColumnToScreen;
        if (doTextWrap) {
            ETextAlign ta = style()->textAlign();
        ...
        for (InlineWalker walker(this); !walker.atEnd(); walker.advance()) {
            ...
            if (o->isReplaced() || o->isFloating() || o->isPositioned()) {
                ...
                if (o->isPositioned())
                    ...
                    else if (layoutState.isFullLayout() || o->needsLayout()) // Replaced elements
        ...
        if (doTextWrap && hasTextToWrap) {
            ...
            if (!isConstrained) {
                ...
                LayoutUnit padding = paddingLeft() + paddingRight();
                if (screenWidth > 0 && width() > (screenWidth + padding)) {
                    ...
                    LayoutUnit maxWidth = screenWidth - 2 * ANDROID_FCTS_MARGIN_PADDING + padding;
