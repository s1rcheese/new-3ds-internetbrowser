...
private:
#if PLATFORM(WKC)
    WKC_DEFINE_GLOBAL_CLASS_OBJ_ENTRY(unsigned, s_scopeCount);
    WKC_DEFINE_GLOBAL_CLASS_OBJ_ENTRY(PropertySetCSSStyleDeclaration*, s_currentDecl);
    WKC_DEFINE_GLOBAL_CLASS_OBJ_ENTRY(bool, s_shouldNotifyInspector);
#else
...
#if ENABLE(MUTATION_OBSERVERS)
#if PLATFORM(WKC)
    WKC_DEFINE_GLOBAL_CLASS_OBJ_ENTRY(bool, s_shouldDeliver);
#else
...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_CLASS_OBJ(unsigned, StyleAttributeMutationScope, s_scopeCount, 0);
WKC_DEFINE_GLOBAL_CLASS_OBJ(PropertySetCSSStyleDeclaration*, StyleAttributeMutationScope, s_currentDecl, 0);
WKC_DEFINE_GLOBAL_CLASS_OBJ(bool, StyleAttributeMutationScope, s_shouldNotifyInspector, false);
#if ENABLE(MUTATION_OBSERVERS)
WKC_DEFINE_GLOBAL_CLASS_OBJ(bool, StyleAttributeMutationScope, s_shouldDeliver, false);
#endif
#else
