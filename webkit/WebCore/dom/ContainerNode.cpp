...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_PTR(NodeCallbackQueue*, s_postAttachCallbackQueue, 0);

WKC_DEFINE_GLOBAL_UINT(s_attachDepth, 0);
WKC_DEFINE_GLOBAL_BOOL(s_shouldReEnableMemoryCacheCallsAfterAttach, false);
#else
...
ContainerNode::~ContainerNode()
{
    if (AXObjectCache::accessibilityEnabled() && documentInternal() && documentInternal()->axObjectCacheExists())
        documentInternal()->axObjectCache()->removeNodeForUse(this);
