...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_PTR(HashSet<Document*>*, documentsThatNeedStyleRecalc, 0);
#else
...
#if ENABLE(WKC_HTTPCACHE)
    else if (equalIgnoringCase(equiv, "cache-control"))
        ResourceHandleManager::sharedInstance()->processHttpEquiv(content, url(), true);
    else if (equalIgnoringCase(equiv, "pragma"))
        ResourceHandleManager::sharedInstance()->processHttpEquiv(content, url(), false);
#endif
}
...
void Document::enqueuePopstateEvent(PassRefPtr<SerializedScriptValue> stateObject)
{
    ...
    if (!domWindow())
        return;
#endif
    enqueueWindowEvent(PopStateEvent::create(stateObject, domWindow() ? domWindow()->history() : 0));
}
