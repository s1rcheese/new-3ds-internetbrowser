...
#if PLATFORM(WKC)
# ifndef NDEBUG
static HashSet<Node*>& ignoreSet_()
{
    WKC_DEFINE_STATIC_PTR(HashSet<Node*>*, set, new HashSet<Node*>);
    return *set;
}
#  define ignoreSet (ignoreSet_())
# endif
#else
...
#if PLATFORM(WKC)
void Node::getNodeCompositeRect(IntRect* rects, int tx, int ty)
{
    for (Node* n = this->firstChild(); n; n = n->traverseNextSibling(this)) {
        RenderObject* o = n->renderer();
        if (o) {
            IntRect rect = o->absoluteBoundingBoxRect(false);
            if (!rect.isEmpty() && !(rect.x() < 0 || rect.y() < 0)) {
                if (n->firstChild()) {
                    n->getNodeCompositeRect(rects, tx, ty);
                }
                rect.move(-tx,-ty);
                rects->unite(enclosingIntRect(rect));
            }
        }
    }
}
...
#if PLATFORM(WKC)
    if (!isDocumentNode() || !hasRareData() || !rareData()->nodeLists()) {
        CRASH();
    }
#endif
    NodeRareData* data = rareData();
