...
#if USE(MPO)
bool matchesMPOSignature(const SharedBuffer& data)
{
...
ImageDecoder* ImageDecoder::create(const SharedBuffer& data, ImageSource::AlphaOption alphaOption, ImageSource::GammaAndColorProfileOption gammaAndColorProfileOption, bool allDataReceived)
{
...
#if USE(MPO)
    if (data.size()<16384 && !allDataReceived)
        return 0;
    if (matchesMPOSignature(data))
        return new MPOImageDecoder(alphaOption, gammaAndColorProfileOption);
    if (matchesJPEGSignature(contents))
        return new JPEGImageDecoder(alphaOption, gammaAndColorProfileOption);
#endif
