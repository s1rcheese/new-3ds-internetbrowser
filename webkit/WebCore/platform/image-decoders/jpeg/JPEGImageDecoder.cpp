...
#if PLATFORM(WKC)
        if (m_scaled == false && info->out_color_space == JCS_RGB) {
            JSAMPLE* jsample = *samples;
            buffer.setRGBLine(0, width, destY, (unsigned char*)jsample, 3);
        } else {
...
#if PLATFORM(WKC)
    bool failed = false;
    unsigned bytesToBeAllocated = 1024;
    if (m_data && (!m_data->needsReallocationToMergeBuffers(&bytesToBeAllocated)
        || wkcMemoryCheckMemoryAllocatablePeer(bytesToBeAllocated, WKC_MEMORYALLOC_TYPE_SHAREDBUFFER_JPEG))) {
        failed = !m_reader->decode(*m_data, onlySize) && isAllDataReceived();
    } else {
        failed = true;
        wkcMemoryNotifyMemoryAllocationErrorPeer(bytesToBeAllocated, WKC_MEMORYALLOC_TYPE_SHAREDBUFFER_JPEG);
    }
    if (failed)
        setFailed();
#else
