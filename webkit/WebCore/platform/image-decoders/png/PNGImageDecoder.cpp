...
public:
...
#if PLATFORM(WKC)
        if (!m_png || !m_info) {
            wkcMemoryNotifyNoMemoryPeer(16384 /* approximate size */);
        }
#endif
    ...
    void createInterlaceBuffer(int size) {
        ...
        ASSERT(!m_interlaceBuffer);
        if (wkcMemoryCheckMemoryAllocatablePeer(size, WKC_MEMORYALLOC_TYPE_IMAGE_PNG)) {
            wkcMemorySetAllocatingForImagesPeer(true);
            WTF::TryMallocReturnValue rv = tryFastMalloc(size);
            wkcMemorySetAllocatingForImagesPeer(false);
            allocSucceeded = rv.getValue(m_interlaceBuffer);
        }
        if (!allocSucceeded) {
            wkcMemoryNotifyMemoryAllocationErrorPeer(size, WKC_MEMORYALLOC_TYPE_IMAGE_PNG);
...
void PNGImageDecoder::rowAvailable(unsigned char* rowBuffer, unsigned rowIndex, int interlacePass)
{
    ...
    if (buffer.status() == ImageFrame::FrameEmpty) {
        ...
        if (!m_reader->hasAlpha())
            buffer.setAllowReduceColor(true);
#endif
    ...
    if (hasAlpha) {
        for (int x = 0; x < width; ++x, pixel += colorChannels) {
            ...
            nonTrivialAlpha |= pixel[3] < 255;
        }
    } else
        buffer.setRGBLine(0, width, y, pixel, colorChannels);
#else
    for (int x = 0; x < width; ++x, pixel += colorChannels) {
        unsigned alpha = hasAlpha ? pixel[3] : 255;
        buffer.setRGBA(x, y, pixel[0], pixel[1], pixel[2], alpha);
...
void PNGImageDecoder::decode(bool onlySize)
{
    ...
    if (!m_data->needsReallocationToMergeBuffers(&bytesToBeAllocated)
        || wkcMemoryCheckMemoryAllocatablePeer(bytesToBeAllocated, WKC_MEMORYALLOC_TYPE_SHAREDBUFFER_PNG)) {
        failed = !m_reader->decode(*m_data, onlySize) && isAllDataReceived();
    } else {
        failed = true;
        wkcMemoryNotifyMemoryAllocationErrorPeer(bytesToBeAllocated, WKC_MEMORYALLOC_TYPE_SHAREDBUFFER_PNG);
    }
    if (failed)
        setFailed();
#else
