...
PassRefPtr<BidiContext> BidiContext::create(unsigned char level, Direction direction, bool override, BidiEmbeddingSource source, BidiContext* parent)
{
    ...
    if (!level) {
        if (!override) {
#if PLATFORM(WKC)
            WKC_DEFINE_STATIC_PTR(BidiContext*, ltrContext, createUncached(0, LeftToRight, false, FromStyleOrDOM, 0).leakRef());
#else
...
#if PLATFORM(WKC)
        WKC_DEFINE_STATIC_PTR(BidiContext*, ltrOverrideContext, createUncached(0, LeftToRight, true, FromStyleOrDOM, 0).leakRef());
#else
    ...
    if (!override) {
#if PLATFORM(WKC)
        WKC_DEFINE_STATIC_PTR(BidiContext*, rtlContext, createUncached(1, RightToLeft, false, FromStyleOrDOM, 0).leakRef());
#else
...
#if PLATFORM(WKC)
    WKC_DEFINE_STATIC_PTR(BidiContext*, rtlOverrideContext, createUncached(1, RightToLeft, true, FromStyleOrDOM, 0).leakRef());
#else
