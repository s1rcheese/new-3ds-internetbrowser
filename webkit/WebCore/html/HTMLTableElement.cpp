...
StylePropertySet* HTMLTableElement::additionalAttributeStyle()
{
    ...
    if (!m_borderAttr && !m_borderColorAttr) {
        ...
        if (m_rulesAttr != UnsetRules) {
#if PLATFORM(WKC)
            WKC_DEFINE_STATIC_PTR(StylePropertySet*, solidBorderStyle, leakBorderStyle(CSSValueHidden));
#else
...
#if PLATFORM(WKC)
        WKC_DEFINE_STATIC_PTR(StylePropertySet*, solidBorderStyle, leakBorderStyle(CSSValueSolid));
#else
...
#if PLATFORM(WKC)
    WKC_DEFINE_STATIC_PTR(StylePropertySet*, outsetBorderStyle, leakBorderStyle(CSSValueOutset));
#else
...
StylePropertySet* HTMLTableElement::additionalGroupStyle(bool rows)
{
    ...
    if (rows) {
#if PLATFORM(WKC)
        WKC_DEFINE_STATIC_PTR(StylePropertySet*, rowBorderStyle, leakGroupBorderStyle(true));
#else
...
#if PLATFORM(WKC)
    WKC_DEFINE_STATIC_PTR(StylePropertySet*, columnBorderStyle, leakGroupBorderStyle(false));
#else
